...
class CalibrationToolbox:
    ...
    def __init__(
        ...
        # self.calibration.set_proposal(self.model)
        # self.calibration.set_sigma_bounds(self.model)

    def run(self):

        for calibration_step in range(5):
            self.simulations.run()
            new_parameter = self.calibration.solve(
                observations=self.observations, simulations=self.simulations
            )

            self.simulations.parameters.state = new_parameter

...
class SequentialMonteCarlo:
    ...
    #: Pandas DataFrame containing data for the posteriors, likelihood, ips, covariant matrices (respectively)
    #: e.g., entries are `["posterior","likelihood","ips","covs"]`
    data: np.array

    #: List of DataFrames collected during the data assimilation
    ...
    @classmethod
    def from_dict(cls: t.Type["SequentialMonteCarlo"], obj: dict):
        """Createa an SequentialMonteCarlo class from a dictionary
    ...
    def get_covariance_matrices(
        self, sigma_guess: float, model: t.Type["Model"]
    ) -> np.array:
    ...
    def get_likelihoods(
        self, model: t.Type["Model"], cov_matrices: np.array
    ) -> np.array:
    ...
    def get_posterors(
        self, model: t.Type["Model"], likelihoods: np.array, posterior_t0: np.array
    ) -> np.array:
    ...
    def get_posterors(
        self, model: t.Type["Model"], likelihoods: np.array, proposal_prev: np.array
    ) -> np.array:
    ...
    def get_effective_sample_size(self, posteriors) -> float:
        eff_all_steps = 1.0 / sum(posteriors**2)
        return eff_all_steps[-1]

    def data_assimilation_loop(
        self, sigma_guess: float, proposal_prev: np.ndarray, model: t.Type["Model"]
    ):
        ...
        self.eff = self.get_effective_sample_size(posteriors=self.posteriors)
...
#         simulations: t.Type["Simulations"],
#     ) -> float:
...
#     def get_ips_covs(
#         self, simulations: t.Type["Simulations"], posterior_current: np.ndarray
